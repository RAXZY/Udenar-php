...
<head>
    ...
    <title>Dashboard - MiUdenar</title>
...
<body>
                ...
                <div class="page-title">
                    <div class="row">
                        <div class="col-12 col-md-6 order-md-1 order-last">
                            ...
                            <p class="text-subtitle text-muted">Añade, edita y elimina registros estudiantiles.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <section class="section">
                    ...
                    <div class="row mb-4 justify-content-center">
                        <div class="col-12 col-md-8 col-lg-6"> 
                            <div class="form-group position-relative has-icon-left">
                                <input type="text" id="buscador" class="form-control form-control-lg rounded-pill shadow-sm" placeholder="  Buscar por código, nombre, apellido o email...">
                    ...
                    <div class="card shadow-sm border-0">
                        <div class="card-header bg-transparent d-flex justify-content-between align-items-center border-bottom">
                            <h5 class="m-0 text-primary">Listado Actual</h5>
            ...
            <div class="modal-content">
                ...
                <form id="form-crear">
                    <div class="modal-body" id="vista-formulario">
                        ...
                        <div class="mb-3">
                            <label>Carrera</label>
                            <select name="degree_id" id="select-carrera-crear" class="form-select" required>
                    ...
                    <div class="modal-body text-center d-none py-5" id="vista-cargando">
                        ...
                        <h5>Guardando en la base de datos...</h5>
                    </div>
                ...
                <form id="form-editar">
                    <div class="modal-body" id="vista-formulario-edit">
                        <div class="mb-3">
                            ...
                            <input type="text" id="edit-code" name="code" class="form-control bg-light" readonly required>
                        </div>
                        <div class="mb-3">
                            <label>Nombre</label>
                        ...
                        <div class="mb-3">
                            <label>Carrera</label>
                            <select name="degree_id" id="select-carrera-editar" class="form-select" required>
                                <option value="">Seleccione una carrera...</option>
                                </select>
                        </div>
                    </div>
    ...
    <script>
        ...
        // =======================================================================
        // CARGAR LAS CARRERAS EN LOS MENÚS DESPLEGABLES
        // =======================================================================
        function cargarCarreras() {
            fetch('api/carreras.php')
                ...
                .then(carreras => {
                    ...
                    const selectCrear = document.getElementById('select-carrera-crear');
                    const selectEditar = document.getElementById('select-carrera-editar');

                    let opcionesHTML = '<option value="">Seleccione una carrera...</option>';

                    carreras.forEach(carrera => {
                        opcionesHTML += `<option value="${carrera.degree_id}">${carrera.degree_name}</option>`;
                    });

                    selectCrear.innerHTML = opcionesHTML;
                    selectEditar.innerHTML = opcionesHTML;
                })
                .catch(error => console.error('Error al cargar carreras:', error));
        }
        // 1. Función principal que hace el GET al API
        ...
        // 2. Función que transforma el JSON en HTML
        function renderTable(datos, tbody) {
            const filasHTML = datos.map(student => {
                // Si tiene carrera la mostramos, si es NULL mostramos "Sin asignar"
                const nombreCarrera = student.degree_name ? student.degree_name : '<span class="text-muted fst-italic">Sin asignar</span>';

                return `
                    <tr>
                        <td class="fw-bold ps-4">#${student.code}</td>
                        <td>${student.first_name} ${student.last_name}</td>
                        <td><span class="text-muted">${student.email}</span></td>
                        <td>${nombreCarrera}</td> <td class="pe-4">
                            <div class="d-flex justify-content-end flex-wrap gap-2">
                                <button class="btn btn-sm btn-outline-warning" 
                                    onclick="abrirModalEditar('${student.code}', '${student.first_name}', '${student.last_name}', '${student.email}', '${student.degree_id}')">
                                    ✏️
                                </button>
                                
                                <button class="btn btn-sm btn-outline-danger" 
                                    onclick="abrirModalBorrar('${student.code}', '${student.first_name}', '${student.last_name}')">
                                    🗑️
                                </button>
                            </div>
                        </td>
        ...
        // ================= LÓGICA PARA CREAR UN ESTUDIANTE (POST) =================
        ...
        formCrear.addEventListener('submit', function(evento) {
            ...
            footerNormal.classList.add('d-none');
            vistaCarga.classList.remove('d-none');
        ...
        createModalEl.addEventListener('hidden.bs.modal', function () {
            ...
            document.getElementById('footer-normal').classList.remove('d-none');
            document.getElementById('vista-cargando').classList.add('d-none');
            document.getElementById('vista-exito').classList.add('d-none');
            document.getElementById('footer-exito').classList.add('d-none');
        });
        
        // ================= LÓGICA PARA EDITAR UN ESTUDIANTE (PUT) =================
        let modalEdicion;
        function abrirModalEditar(code, firstname, lastname, email, degree_id) {
            
            document.getElementById('edit-code').value = code;
            document.getElementById('edit-firstname').value = firstname;
            document.getElementById('edit-lastname').value = lastname;
            document.getElementById('edit-email').value = email;
            document.getElementById('select-carrera-editar').value = (degree_id === 'null' || degree_id === 'undefined') ? '' : degree_id;

            document.getElementById('vista-formulario-edit').classList.remove('d-none');
            document.getElementById('footer-normal-edit').classList.remove('d-none');
            document.getElementById('vista-cargando-edit').classList.add('d-none');
            document.getElementById('vista-exito-edit').classList.add('d-none');
            document.getElementById('footer-exito-edit').classList.add('d-none');
        ...
        formEditar.addEventListener('submit', function(evento) {
            evento.preventDefault(); 
            const vistaForm = document.getElementById('vista-formulario-edit');
            const vistaCarga = document.getElementById('vista-cargando-edit');
            const vistaExito = document.getElementById('vista-exito-edit');
            const footerNormal = document.getElementById('footer-normal-edit');
            const footerExito = document.getElementById('footer-exito-edit');
        ...
        // ================= LÓGICA PARA BORRAR UN ESTUDIANTE (DELETE) =================
        let modalBorrado;
        function abrirModalBorrar(code, firstname, lastname) {
            document.getElementById('delete-code').value = code;
            document.getElementById('delete-nombre-display').innerText = firstname + ' ' + lastname;

            document.getElementById('vista-formulario-delete').classList.remove('d-none');
            document.getElementById('footer-normal-delete').classList.remove('d-none');
            document.getElementById('vista-cargando-delete').classList.add('d-none');
            document.getElementById('vista-exito-delete').classList.add('d-none');
            document.getElementById('footer-exito-delete').classList.add('d-none');
        ...
        formBorrar.addEventListener('submit', function(evento) {
            evento.preventDefault();
            const vistaForm = document.getElementById('vista-formulario-delete');
            const vistaCarga = document.getElementById('vista-cargando-delete');
            const vistaExito = document.getElementById('vista-exito-delete');
            const footerNormal = document.getElementById('footer-normal-delete');
            const footerExito = document.getElementById('footer-exito-delete');
        ...
        inputBuscador.addEventListener('input', function(evento) {
            ...
            const tbody = document.getElementById('tabla-estudiantes');
            const estudiantesFiltrados = todosLosEstudiantes.filter(student => {
                const datosEstudiante = `${student.code} ${student.first_name} ${student.last_name} ${student.email}`.toLowerCase();
                return datosEstudiante.includes(textoBuscado);
            });
            renderTable(estudiantesFiltrados, tbody);
        });
